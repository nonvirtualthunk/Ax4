...
object DeckData extends Clazz[DeckData]("DeckData", classOf[DeckData]){
	...
	val expendedPile = Field.fromValue(Sentinel.expendedPile).createField[DeckData]("expendedPile",f => f.expendedPile, (f,expendedPile) => f.expendedPile = expendedPile, DeckData) 
	fields += "expendedPile" -> expendedPile
	...
	def copyInto(from : DeckData, to : DeckData) {
		...
		to.expendedPile = from.expendedPile
		to.attachedCards = from.attachedCards
...
object Skill extends Clazz[Skill]("Skill", classOf[Skill]){
	...
	val cardRewards = Field.fromValue(Sentinel.cardRewards).createField[Skill]("cardRewards",f => f.cardRewards, (f,cardRewards) => f.cardRewards = cardRewards, Skill) 
	fields += "cardRewards" -> cardRewards
