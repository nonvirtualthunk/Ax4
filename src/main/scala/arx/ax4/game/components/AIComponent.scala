...
import arx.ax4.game.event.TurnEvents.{EntityTurnStartEvent, TurnStartedEvent}
import arx.ax4.game.logic.{AllegianceLogic, AxPathfinder, CharacterLogic, CombatLogic, IdentityLogic, MovementLogic}
...
class AIComponent extends GameComponent {
	...
	override protected def onInitialize(world: World): Unit = {
		...
		onGameEventEndWithPrecedence(-100) {
			case EntityTurnStartEvent(entity, _) if !AllegianceLogic.isPlayerCharacter(entity) =>
				takeAITurn(entity)(world)
		}
	}

	def takeAITurn(entity: Entity)(implicit world: World) = {
		implicit val view = world.view
